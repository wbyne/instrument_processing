...
#  cp ${file} ${file}.tmp
  ...
  cp ${file}.particle.aud ${file}.particle.aud.tmp
#  echo "#restart" $andthen > ${file}
  echo "#restart" $andthen > ${file}.particle
  echo "#restart" $andthen > ${file}.particle.aud
  # restart screen 
...
#  mv ${filedir}/${file2}.tmp ${filedir}/archive/${file2}.bkup_$nowdy$nowmonth$nowyr\_$nowhr$nowmin
  ...
  mv ${filedir}/${file2}.particle.aud.tmp ${filedir}/archive/${file2}.particle.aud.bkup_$nowdy$nowmonth$nowyr\_$nowhr$nowmin
#  gzip ${filedir}/archive/${file2}.bkup_$nowdy$nowmonth$nowyr\_$nowhr$nowmin
  ...
  gzip ${filedir}/archive/${file2}.particle.aud.bkup_$nowdy$nowmonth$nowyr\_$nowhr$nowmin
...
#crontab's: 
#gpoz
#*/15 * * * * /mnt/space/workspace/instrument_processing/scripts/0_script_runner.sh > /dev/null
#3,18,33,48 * * * * /mnt/space/workspace/instrument_processing/scripts/0_script_runner.Sav.sh > /dev/null
#gis_edit
#2,17,28,48 * * * * /mnt/space/workspace/instrument_processing/scripts/8_update_sql.pl > /dev/null
#8,23,38,53 * * * * /mnt/space/workspace/instrument_processing/scripts/8_update_sql.pl > /dev/null
