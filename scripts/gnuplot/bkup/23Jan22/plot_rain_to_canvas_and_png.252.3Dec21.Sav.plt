...
set xdata time
set xrange [*:time(0) - 252*24*60*60]
...
set terminal canvas standalone mousing size 900,1100 jsdir 'http://www.rainfalldata.com/rainfall/resources/gnuplot/'
set output htmlfilename
#set terminal qt
if ((exists("rain_name"))&&(exists("depth_name"))&&(exists("temp_name"))&&(exists("wtr_tmp_name"))&&(exists("cond_name"))&&(exists("tmp_rtc_name"))) {
unset key
set multiplot layout 7,1 rowsfirst upwards                             
set lmargin 14
set bmargin 8
set grid
set tics out
...
set xrange [time(0) - 252*24*60*60:*] #without the *, it defaults to last max or min, which we set earlier while trying to find the max/min value
set ytics nomirror
set ylabel "Rain(in)"
...
#set xrange [*:time(0) - 203*24*60*60]
