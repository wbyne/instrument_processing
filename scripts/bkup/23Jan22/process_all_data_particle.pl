...
#	}
	...
	for ($j=0; $j<=$#tmp; $j++) { #traverse the array looking for data
	  if ($tmp[$j] =~ /^data/) { #if it starts with the word data...
		...
		$new_data[$i]=$tmp2[1].','.$tmp2[2].','.$tmp2[3].','.$tmp2[4].','.$tmp2[5].','.$tmp2[6].','.$tmp2[7].','.$tmp2[8].','.$tmp2[9].','.$tmp2[10].','.$tmp2[11].','.
		$i++;
	    }
	  }
	}
...
S		#	print STAT_PARM_SUMM "$tmp2[0],$tmp2[8],$tmp2[9],$tmp2[10],$tmp2[11],$tmp2[12],$tmp2[13],$tmp2[14],$tmp2[15]\n";
		}
		close (STAT_PARM);
                initialize_reset();		
}
