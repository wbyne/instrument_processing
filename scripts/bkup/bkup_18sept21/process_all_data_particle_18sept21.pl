...
#	}
	...
	for ($j=0; $j<=$#tmp; $j++) { #traverse the array looking for data
	  if ($tmp[$j] =~ /^data/) { #if it starts with the word data...
		...
		if ($#tmp2 != 9) { next; } #skipping hash data and everything else
		while ($tmp2[9] =~ /\\n$/) {  #remove multiple nl\cr, etc....
		  $tmp2[9] =~ s/\\n//;
		}#chomp $tmp2[9];
		#$tmp[$j] =~ s/:/,/g; #substitute commas for the colons.  helps later -wb-8july17
		$new_data[$i]=$tmp2[1].','.$tmp2[2].','.$tmp2[3].','.$tmp2[4].','.$tmp2[5].','.$tmp2[6].','.$tmp2[7].','.$tmp2[8].','.$tmp2[9].','.
		$i++;
	    }
	  }
	}
...
for ($i=0; $i<=$#condensed_data; $i++) {
	...
	$wtr_tmp[$i]       =$alldata[8];	    
	$alldatastring[$k] =  $fakestation[$k].",".$timestring[$k].",".$station[$k].",".$month[$k].",".$day[$k].",".$year[$k].",".$hour[$k].",".$minute[$k].",".$month[$k]."/".$day[$k]."/".$year[$k]." ".$hour[$k].":".$minute[$k].",".$month[$k]."/".$day[$k]."/".$year[$k]." ".$hour[$k].":".$fakeminute[$k].",".$rain[$k].",".$velo[$k].",".$temp[$k].",".$depth[$k].",".$volt[$k].",".$temp_rtc[$k].",".$wtr_tmp[$k];
...
##			$_ =~ s/^\s+//; #from https://perlmaven.com/trim 24jun17_1446.  Successive reads and writes on the data files ends up multiplying errant spaces
		...
		open (STAT_PARM, ">","$savepath/data/site/$station_list[$i]/ALLDATA/$station_list[$i]"."_"."ALLDATA.csv") or warn "Can't open $station_list[$i]_ALLDATA.csv\n";
	        print STAT_PARM "#Station,TimeStamp,StationCtrlCode,Month,Day,Year,Hour,Minute,Date_Time,Date_Time_Adj,Rain_hundredths,Velo_fps,Temp_F,Depth_Hundredths_of_Feet,Voltage_V,Temp_RTC_F,WTR_TMP_F\n";
