...
#### Here is the problem: if the station hasn't reported in a time interval, and the alarm is set to 
####  MIN, then the missed measurement (0) is less than the alarm almost always, so it triggers an 
####  alert.  Mostly caused by voltage requests.  Not sure that this sub needs to check all lower subroutines
####  or time intervals as well.  little more work to do...-fwb-20Sept22_2315
		    if ($l == 2) {
			if ($param[$i][$j][$k][$l]<$alarm[$i][$j][$k][$l]) {
